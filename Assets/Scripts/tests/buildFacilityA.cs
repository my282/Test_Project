...
public class build_FacilityA : MonoBehaviour
{
    ...
    void RegisterAutoProduction()
    {
        ...
        if (facilityA == null)
        {
            Debug.LogError("FacilityAが見つかりません！");
            return;
        }
        
        if (!facilityA.isUnlocked)
        {
            Debug.LogError("FacilityAが解放されていません！");
            return;
        }
        
        // デバッグ: 生成設定を確認
        Debug.Log($"=== FacilityA 生成設定確認 ===");
        ...
        if (facilityA.productionConfig.enableAutoProduction)
        {
            ...
            if (FacilityProductionController.Instance != null)
            {
                ...
                Debug.Log($"✅ FacilityAの自動生成を開始: {productionInfo}");
            }
            else
            {
        ...
        {
            Debug.Log("❌ FacilityAの自動生成は無効になっています（FacilityDataで Enable Auto Production をONにしてください）");
        }
    }
    ...
    void DebugProductionStatus()
    {
        ...
        {
            Debug.LogError("[FacilityA] 見つかりません");
        }
